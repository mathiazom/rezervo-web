...
import { ChainId, ClassId, brandChainId } from "@/types/brand";
...
export function useOpenClassInfo() {
    ...
    return (chainIdentifier: ChainId, classId: ClassId) => {
        void navigate({
            search: (prev) => ({
                ...prev,
                [CLASS_ID_QUERY_PARAM]: classId,
                [CLASS_CHAIN_QUERY_PARAM]:
                    chainIdentifier === brandChainId(viewedChainIdentifier) ? undefined : chainIdentifier,
            }),
            replace: true,
        });
    };
...
export function useClassInfo() {
    ...
    const navigate = routeApi.useNavigate();
    const chainIdentifier = classChainIdentifier ?? brandChainId(viewedChainIdentifier);
