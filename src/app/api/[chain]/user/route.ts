...
export const GET = withApiAuthRequired(async (req, ctx) => {
    ...
    return await doOperation(() => get(`${process.env["CONFIG_HOST"]}/${chainIdentifier}/user`, accessToken));
});
...
export const PUT = withApiAuthRequired(async (req, ctx) => {
    ...
    return await doOperation(() => put(`${process.env["CONFIG_HOST"]}/${chainIdentifier}/user`, accessToken, data));
});
