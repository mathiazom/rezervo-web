...
const Config = ({
  ...
  const selectedClassesConfig =
    selectClassIds.length > 0
      ? `classes:\n  ${allClassesConfigs
          .filter(([classId]) => selectClassIds.includes(String(classId)))
          .map(([, config]) => config)
          .join("\n  ")}`
      : "";
  ...
  return selectClassIds.length > 0 ? (
    <Box
      sx={{
        position: "relative",
    ...
    <Box>
      <Alert severity="info">
        Select classes to the left to generate your <code>config.yaml</code>
      </Alert>
    </Box>
  );
};
